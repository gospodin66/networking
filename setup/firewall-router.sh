...
#iptables -A INPUT -p tcp -m state --state NEW -m limit --limit 50/second --limit-burst 150 -j ACCEPT
...
# Allow established and related IN connections
iptables -A INPUT -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
# Allow established and related OUT connections
...
iptables -A INPUT -p tcp --syn --dport ${port["imap"]} -m connlimit --connlimit-above 15 --connlimit-mask 32 -j REJECT --reject-with tcp-reset
...
iptables -A INPUT -m state --state RELATED,ESTABLISHED -m limit --limit 20/second --limit-burst 300 -j ACCEPT
...
echo -e "LOCAL PORT RANGE: $port_range_min:$port_range_max\n"
