...
- `ssh_tunnel` - (uses pubkey|password auth) opens SSH tunnel via jump server -- `./ssh_tunnel .config <pubkey|password>`
- `tunnel_check.sh` - base tunnel script -- `./tunnel_check.sh .tconfig`


#### Connect via tunnel
Following command connects `ssh-user` to `ssh-dest` via `ssh-jump`:
```bash
$ docker exec -it -u tunnel -w /home/tunnel ssh-user ssh -p 1089 dest@localhost
```

#### SCP
Following command transfers files via tunnels:
```bash
scp dest@172.17.0.4:/home/dest/sshtestfile recv.txt
```

#### Run within network
Tunnel scripts can be used to connect from `dest` host to nodes in `net1` & `net2` via `net-router` container. \
User is prompted to enter passphrase twice (1 for generation, 1 for usage during ssh-key upload to jump server)
...
$ docker exec -it -u tunnel -w /home/tunnel ssh-user ssh -p 21122 tunneller@172.17.0.4
$ docker exec -it -u tunnel -w /home/tunnel ssh-user ssh -p 21222 tunneller@172.17.0.4
```