...
- 1 router network (`router-net`) -> used to forward all traffic. 
    - Router network initially consists of:
        ...
        - Container `ssh-dest` connects to it to forward traffic to private networks.
...
Consists of:
- 2 ssh servers (`ssh-jump` & `ssh-dest`) -> jump server tunnels to dest server
- 1 client      (`ssh-user`)              -> **starting point**. 
...
When running `ssh-*` tunnel containers, `default docker network` is added into topology.
...
```bash 
   ...
   |f          |t             |    |-------------|  |-------------|  |-------------|    |
   |f          |r             |    |  ssh-user   |  |   ssh-jump  |  |   ssh-dest  |    |
   |i          |a             |    |-------------|  |---+++++++---|  |-------------|    |
   |c          |f             |            |            |     |            |            |
   |           |f             |            |-------------     -------------|            |
   |           |i             |                         tunnel                          |
